...
class GameState(TypedDict):
    ...
    npc_stats : dict[str, dict[str, int]]
        ...
        값: {수치 항목명: 현재 값(0~100)}
        수치 항목(trust, hostility 등)은 config.py의 DEFAULT_NPC_STATS에서 초기화.
        예시: {"김도현": {"trust": 20, "hostility": 70}, ...}

    stats_locked : bool
        True이면 npc_stats 수정 불가 (Phase 2 진입 후 확정된 상태).
        button_node.py에서 finalize_stats() 호출 시 True로 전환.
    ...
    is_dead : bool
        True이면 유저가 사망한 상태. loop_node.py에서 루프 리셋을 트리거.
